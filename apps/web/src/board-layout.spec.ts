...
describe('layoutBoard', () => {
  ...
  test('an open issue with no axis label lands in No status', () => {
    ...
    expect(
      columns.find(c => c.title === 'No status')?.cards.map(c => c.id)
    ).toEqual([1])
  })
  ...
  test('a closed issue lands in Done regardless of its labels', () => {
    ...
    expect(columns.find(c => c.title === 'Done')?.cards.map(c => c.id)).toEqual(
      [1]
    )
  })
  ...
  test('places a card by the first axis label it carries, in axis order', () => {
    ...
    expect(
      columns.find(c => c.title === 'To Do')?.cards.map(c => c.id)
    ).toEqual([1])
