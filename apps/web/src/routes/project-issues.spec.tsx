...
async function openSeededIssues() {
  ...
  const { client, router, user } = await renderApp(async (client) => {
    ...
    assigneeId = expectId(await (await client.api.actors.$post({ json: { name: 'Ada', kind: 'human' } })).json())
...
describe('project issues list', () => {
  test('renders the issue as a table row with its columns', async () => {
    ...
    const row = (await screen.findByRole('button', { name: /Open DEMO-1 Wire the list/ })).closest('tr')
    if (row === null) {
  ...
  test('a row click opens the shared detail modal', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: /Open DEMO-1 Wire the list/ }))
    ...
    expect(screen.getByLabelText<HTMLInputElement>('Title').value).toBe('Wire the list')
  })
  ...
  test('an issue created via the API appears live with no reload', async () => {
    ...
    expect(await screen.findByRole('button', { name: /Open DEMO-2 Live row/ })).toBeDefined()
  })
  ...
  test('an issue deleted via the API drops its row live', async () => {
    ...
    await client.api.projects[':slug'].issues[':number'].$delete({ param: { slug, number: String(1) } })

    await waitFor(() => {
      expect(screen.queryByRole('button', { name: /Open DEMO-1 Wire the list/ })).toBeNull()
    })
  })
