...
const parse = (search: Record<string, unknown>) =>
  toFilters(filterSearchSchema.parse(search))
...
describe('filterSearchSchema + toFilters', () => {
  ...
  test('a malformed value degrades to inactive instead of throwing', () => {
    ...
    expect(
      parse({ type: 'not-an-array', label: 'x', assignee: {}, blocked: 'yes' })
...
describe('matchesFilters', () => {
  ...
  test('label is OR within the axis (any matching label)', () => {
    ...
    expect(
      matchesFilters(issue({ labels: [{ id: 2 }, { id: 9 }] }), filters)
    ).toBe(true)
  ...
  test('assignee matches a specific actor, Unassigned, or Any', () => {
    expect(
      matchesFilters(issue({ assigneeId: 5 }), { ...emptyFilters, assignee: 5 })
    ).toBe(true)
    expect(
      matchesFilters(issue({ assigneeId: 6 }), { ...emptyFilters, assignee: 5 })
    ).toBe(false)
    expect(
      matchesFilters(issue({ assigneeId: null }), {
        ...emptyFilters,
        assignee: 'unassigned'
      })
    ).toBe(true)
    expect(
      matchesFilters(issue({ assigneeId: 5 }), {
        ...emptyFilters,
        assignee: 'unassigned'
      })
    ).toBe(false)
  ...
  test('blocked and ready toggles restrict to that derived state', () => {
    expect(
      matchesFilters(issue({ blocked: true }), {
        ...emptyFilters,
        blocked: true
      })
    ).toBe(true)
    expect(
      matchesFilters(issue({ blocked: false }), {
        ...emptyFilters,
        blocked: true
      })
    ).toBe(false)
    expect(
      matchesFilters(issue({ ready: true }), { ...emptyFilters, ready: true })
    ).toBe(true)
    expect(
      matchesFilters(issue({ ready: false }), { ...emptyFilters, ready: true })
    ).toBe(false)
  })
  ...
  test('axes combine with AND', () => {
    ...
    expect(
      matchesFilters(issue({ type: 'bug', labels: [{ id: 1 }] }), filters)
    ).toBe(true)
    // Right type, wrong label -> excluded (the label axis is not satisfied).
    expect(
      matchesFilters(issue({ type: 'bug', labels: [{ id: 2 }] }), filters)
    ).toBe(false)
  })
})
...
describe('isFilterActive', () => {
  test('false for the empty set, true once any axis is set', () => {
    ...
    expect(isFilterActive({ ...emptyFilters, assignee: 'unassigned' })).toBe(
      true
    )
...
describe('toSearchValues', () => {
  test('omits inactive axes so the URL stays clean and round-trips', () => {
    ...
    const active: typeof emptyFilters = {
      ...emptyFilters,
      type: ['bug'],
      label: [1],
      assignee: 5,
