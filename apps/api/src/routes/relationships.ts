...
const BlockedByParamSchema = IssueParamSchema.extend({ blockerNumber: idParam('blockerNumber') })
...
const reachesViaParent = (db: Db, startId: number, targetId: number): boolean => {
...
const reachesViaBlockers = (db: Db, startId: number, targetId: number): boolean => {
  const seen = new Set<number>()
  const stack = [startId]
...
const setParentRoute = createRoute({
  ...
  request: {
    ...
    body: { content: { 'application/json': { schema: SetParentSchema } }, required: true },
  },
  responses: {
...
const blockRoute = createRoute({
  ...
  summary: 'Declare a blocked-by edge (idempotent; rejects self-blocks and cycles)',
  request: { params: BlockedByParamSchema },
  responses: {
    200: jsonBody(IssueSchema, 'The blocked issue with its updated derived state'),
    400: jsonBody(ErrorSchema, 'An issue cannot block itself'),
    404: jsonBody(ErrorSchema, 'No such project, issue, or blocker'),
    409: jsonBody(ErrorSchema, 'This edge would create a cycle in the blocking graph'),
  },
})
  ...
  return new OpenAPIHono({
    ...
    .openapi(setParentRoute, (c) => {
      ...
      if (reachesViaParent(db, parent.id, issue.id)) {
        return c.json({ error: 'Setting this parent would create a cycle' }, 409)
      }
      const row = db
        .update(issues)
    ...
    .openapi(blockRoute, (c) => {
      ...
      if (reachesViaBlockers(db, blocker.id, issue.id)) {
        return c.json({ error: 'This edge would create a cycle in the blocking graph' }, 409)
      }
    ...
    .openapi(unblockRoute, (c) => {
      ...
      db.delete(issueBlockedBy)
        .where(and(eq(issueBlockedBy.issueId, issue.id), eq(issueBlockedBy.blockerId, blocker.id)))
        .run()
      const snapshot = toIssue(db, issue, project.key)
      bus.publishIssueChanged(project.id, snapshot)
      return c.json(snapshot, 200)
