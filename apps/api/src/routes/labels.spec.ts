...
const createLabel = async (slug: string, name: string) =>
  app.request(`/api/projects/${slug}/labels`, { method: 'POST', ...json({ name }) })

const listLabels = async (slug: string) =>
  z.array(LabelSchema).parse(await (await app.request(`/api/projects/${slug}/labels`)).json())

const parseLabel = async (res: Response) => LabelSchema.parse(await res.json())
const parseLabels = async (res: Response) => z.array(LabelSchema).parse(await res.json())

const attach = async (slug: string, number: number, labelId: number) =>
  app.request(`/api/projects/${slug}/issues/${number}/labels/${labelId}`, { method: 'PUT' })

const detach = async (slug: string, number: number, labelId: number) =>
  app.request(`/api/projects/${slug}/issues/${number}/labels/${labelId}`, { method: 'DELETE' })

const getIssue = async (slug: string, number: number) =>
  IssueSchema.parse(await (await app.request(`/api/projects/${slug}/issues/${number}`)).json())
...
describe('POST /api/projects/:slug/labels', () => {
  ...
  test('creates a label', async () => {
    ...
    expect(await parseLabel(res)).toMatchObject({ name: 'blocked', id: expect.any(Number) })
  })
  ...
  test.each([
    ...
    const res = await app.request('/api/projects/demo/labels', { method: 'POST', ...json(body) })
...
describe('GET /api/projects/:slug/labels', () => {
  test('lists a project labels ordered by name', async () => {
    ...
    expect((await listLabels('demo')).map((l) => l.name)).toEqual(['alpha', 'mu', 'zeta'])
  })
...
describe('attach / detach labels to issues', () => {
  ...
  test('attaches multiple labels and the issue read includes them', async () => {
    ...
    expect((await parseLabels(await attach('demo', 1, a.id))).map((l) => l.name)).toEqual(['alpha'])
    expect((await parseLabels(await attach('demo', 1, b.id))).map((l) => l.name)).toEqual([
      'alpha',
      'beta',
    ])
...
describe('cascade behaviour', () => {
  test('deleting a label detaches it from issues but keeps other labels', async () => {
    ...
    const res = await app.request(`/api/projects/demo/labels/${doomed.id}`, { method: 'DELETE' })
    expect(res.status).toBe(204)

    expect((await listLabels('demo')).map((l) => l.name)).toEqual(['kept'])
    expect((await getIssue('demo', 1)).labels.map((l) => l.name)).toEqual(['kept'])
  })
...
describe('per-project SSE emissions', () => {
  ...
  test('emits label.deleted and issue.changed (converged snapshot) on delete', async () => {
    ...
    const { events, controller } = await openStream('demo')
    await app.request(`/api/projects/demo/labels/${label.id}`, { method: 'DELETE' })
  ...
  test('emits label.changed and a converged issue.changed on rename', async () => {
    ...
    const issue = await nextEventOfType(events, 'issue.changed')
    expect(IssueSchema.parse(issue.data).labels.map((l) => l.name)).toEqual(['new'])
    controller.abort()
  })
})
